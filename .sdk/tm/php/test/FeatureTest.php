...
class FtCtrl extends AdviceSlipControl
{
...
class FtClient
{
    ...
    private AdviceSlipUtility $utility;

    public function __construct(AdviceSlipUtility $utility, array $options)
    {
    ...
    public function get_utility(): AdviceSlipUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AdviceSlipContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new AdviceSlipUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (AdviceSlipContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AdviceSlipConfig::make_feature($name);
    ...
    private static function buildUrl(AdviceSlipSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AdviceSlipError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AdviceSlipResponse($fetched);
        }
    ...
    private function fail(AdviceSlipContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AdviceSlipContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
