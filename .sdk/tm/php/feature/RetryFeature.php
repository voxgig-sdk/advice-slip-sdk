...
class AdviceSlipRetryFeature extends AdviceSlipBaseFeature
{
    ...
    public function init(AdviceSlipContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AdviceSlipContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AdviceSlipContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AdviceSlipContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
