...
type Args struct {
	Entity string         `json:"entity" jsonschema:"advice | search"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.AdviceSlipSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "advice-slip_list",
		Description: "List records from AdviceSlip. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "advice-slip_load",
		Description: "Load a single record from AdviceSlip. " +
...
func runOp(client *sdk.AdviceSlipSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.AdviceSlipSDK, name string) (sdk.AdviceSlipEntity, error) {
	switch strings.ToLower(name) {
	case "advice":
		return client.Advice(nil), nil
	case "search":
