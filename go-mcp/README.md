...
[MCP](https://modelcontextprotocol.io) server exposing the AdviceSlip SDK as
two agent tools — `advice-slip_list` and `advice-slip_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/advice-slip-mcp)
make build

# 2. Provide credentials via the environment
export ADVICE_SLIP_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user advice-slip \
  -- /absolute/path/to/advice-slip-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./advice-slip-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// advice-slip_list: first page of records
{ "entity": "advice" }
{ "entity": "advice", "query": { } }

// advice-slip_load: one record by id
{ "entity": "advice", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/advice-slip-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user advice-slip \
     -- "$PWD"/dist/*/advice-slip-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `advice-slip_list` and `advice-slip_load` tools now appear
   in new sessions. Ask the agent to *"list advice using advice-slip"*
   and it calls `advice-slip_list` with `{"entity":"advice"}`.
...
```sh
export ADVICE_SLIP_APIKEY=sk_live_xxx            # API key
export ADVICE_SLIP_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `advice-slip_list` tool
...
### Call the `advice-slip_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `advice-slip_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `advice-slip_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ADVICE_SLIP_APIKEY` | API key sent with every request. |
| `ADVICE_SLIP_BASE` | Optional override of the API base URL. |
...
```sh
./advice-slip-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"advice-slip_load","arguments":{"entity":"advice","query":{"id":1}}}}'
```
