...
The Lua SDK for the AdviceSlip API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Advice()` — each with the same small set of operations (`load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/advice-slip-sdk/releases)),
...
```
lua/
├── advice-slip_sdk.lua    -- Main SDK module
...
The main module (`advice-slip_sdk`) exports the SDK constructor
