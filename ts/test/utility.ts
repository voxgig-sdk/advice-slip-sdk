...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.ADVICESLIP_TEST_LIVE ||
    'TRUE' === process.env.ADVICESLIP_TEST_OVERRIDE
  ) {
  ...
  m.ADVICESLIP_TEST_EXPLAIN = process.env.ADVICESLIP_TEST_EXPLAIN || m.ADVICESLIP_TEST_EXPLAIN
