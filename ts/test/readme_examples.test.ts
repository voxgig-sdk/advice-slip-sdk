...
//     (\`new AdviceSlipSDK(...)\` / \`AdviceSlipSDK.test(...)\` become
//     \`AdviceSlipSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"advice":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AdviceSlipSDK(...)\` and any
// \`AdviceSlipSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AdviceSlipSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AdviceSlipSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
