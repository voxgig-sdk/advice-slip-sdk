...
class AdviceSlipNetsimFeature extends AdviceSlipBaseFeature
{
    ...
    private function _simulate(AdviceSlipContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(AdviceSlipContext $ctx, array $applied): void
    {
        $client = $this->client;
