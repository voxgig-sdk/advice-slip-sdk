...
class AdviceSlipMakeError
{
    public static function call(?AdviceSlipContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new AdviceSlipContext([], null);
        }
        $op = $ctx->op ?? new AdviceSlipOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new AdviceSlipResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof AdviceSlipError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new AdviceSlipError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof AdviceSlipError) {
