...
class SearchEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ADVICESLIP_TEST_SEARCH_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def search_basic_setup(extra)
  ...
  entid_env_raw = ENV["ADVICESLIP_TEST_SEARCH_ENTID"]
  ...
  if env["ADVICESLIP_TEST_LIVE"] == "TRUE"
    ...
    client = AdviceSlipSDK.new(Helpers.to_map(merged_opts))
  end
