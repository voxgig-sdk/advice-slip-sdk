...
module AdviceSlipUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AdviceSlipResponse.new({})
    ...
    if fetch_err
      ...
      response = AdviceSlipResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AdviceSlipResponse.new(fetched)
    else
