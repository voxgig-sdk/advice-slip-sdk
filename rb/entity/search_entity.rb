...
class SearchEntity
  ...
  def data_set(args)
    if args
      @_data = AdviceSlipHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = AdviceSlipHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Search, Hash] the loaded Search; raises AdviceSlipError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = AdviceSlipHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
