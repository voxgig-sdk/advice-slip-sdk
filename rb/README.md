...
The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Advice` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
require_relative "AdviceSlip_sdk"
client = AdviceSlipSDK.new(options)
```
...
```ruby
client = AdviceSlipSDK.test(testopts, sdkopts)
```
...
raise a `AdviceSlipError` (a `StandardError` subclass), so wrap
...
```
rb/
├── AdviceSlip_sdk.rb       -- Main SDK module
...
The main module (`AdviceSlip_sdk`) exports the SDK class
