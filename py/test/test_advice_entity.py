...
class TestAdviceEntity:

    def test_should_create_instance(self):
        testsdk = AdviceSlipSDK.test(None, None)
        ent = testsdk.Advice(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _advice_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "advice." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ADVICESLIP_TEST_ADVICE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        advice_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.advice")))
        advice_ref01_data = None
        if len(advice_ref01_data_raw) > 0:
...
def _advice_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/advice/AdviceTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("ADVICESLIP_TEST_ADVICE_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("ADVICESLIP_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
                "apikey": env.get("ADVICESLIP_APIKEY"),
            },
            extra or {},
        ])
        client = AdviceSlipSDK(helpers.to_map(merged_opts))

    _live = env.get("ADVICESLIP_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("ADVICESLIP_TEST_EXPLAIN") == "TRUE",
        "live": _live,
