...
class AdviceSlipTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AdviceSlipTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AdviceSlipTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AdviceSlipTestRunner.getenv("ADVICESLIP_TEST_LIVE")
        override = AdviceSlipTestRunner.getenv("ADVICESLIP_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AdviceSlipTestRunner.getenv(key)
        ...
        explain = AdviceSlipTestRunner.getenv("ADVICESLIP_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["ADVICESLIP_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if AdviceSlipTestRunner._test_control is not None:
            return AdviceSlipTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AdviceSlipTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AdviceSlipTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AdviceSlipTestRunner.load_test_control()
...
def load_env_local():
    AdviceSlipTestRunner.load_env_local()


def env_override(m):
    return AdviceSlipTestRunner.env_override(m)


def entity_list_to_data(lst):
    return AdviceSlipTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AdviceSlipTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AdviceSlipTestRunner.load_test_control()


def live_delay_ms():
    return AdviceSlipTestRunner.live_delay_ms()
