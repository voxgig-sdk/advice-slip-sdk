...
AQL-driven command-line client **and** interactive REPL for the AdviceSlip
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/advice-slip-cli)
make build

# 2. See usage (words, entities, env vars)
./advice-slip-cli --help
...
./advice-slip-cli load 1 advice            # {id:1} shorthand
./advice-slip-cli load '{id:1}' advice       # explicit match map

# 5. Override the API base URL for a single call
ADVICE_SLIP_BASE=https://api.example.com ./advice-slip-cli load 1 advice

# 6. No arguments -> interactive REPL
./advice-slip-cli
advice-slip> load 1 advice
advice-slip> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/advice-slip-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/advice-slip-cli load 1 advice
   ```
...
```sh
./advice-slip-cli load 1 advice          # scalar shorthand for {id:1}
./advice-slip-cli load '{id:1}' advice     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `advice-slip>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `advice`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/advice-slip-cli`. |
...
(`./advice-slip-cli <expr>`) and interactively (the REPL), and expressions compose the
