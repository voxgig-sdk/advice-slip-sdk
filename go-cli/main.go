// Command AdviceSlip-cli is a CLI + REPL for the AdviceSlip SDK,
...
const prompt = "advice-slip"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "advice search"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      ADVICE_SLIP_APIKEY (api key), ADVICE_SLIP_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: ADVICE_SLIP_APIKEY carries the API key and
	// ADVICE_SLIP_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ADVICE_SLIP_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ADVICE_SLIP_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAdviceSlipSDK(opts)
